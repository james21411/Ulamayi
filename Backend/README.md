...
Toutes ces routes sont **publiques** pas e token
...
- **Body JSON :**
  ```json
  {
    ...
    "role": "eleve",  // Peut etre: "eleve", "enseignant", "parent"
...
- **Body JSON :**
  ...
  ```
- **Réponse (200) :** Retourne le profil utilisateur ET le `token`. **il faut que le token soit e sauvé coté Front **.
  ```json
  {
    "token": "eyJhbG",
